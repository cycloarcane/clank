...
setup_configuration() {
    ...
    if [[ ! -f "$SECRETS" ]]; then
        ...
        print_warning "Created ESP32LEDs/secrets.h from the template — set MQTT_USER/MQTT_PASS in it."
    fi
    ...
    # Create the runtime env file. The MQTT password is the only secret here;
    # everything device-related is now configured per-device in config/devices.yaml.
...
EOF
    ...
    if [[ -z "$MQTT_PASS" ]]; then
        print_warning "MQTT_PASS is empty — set it in ESP32LEDs/secrets.h and re-run, or edit .env."
    fi
}
...
print_completion_info() {
    ...
    echo "1. Set MQTT creds in ${YELLOW}ESP32LEDs/secrets.h${NC} (MQTT_USER/MQTT_PASS), then"
    echo "   set up the broker: ${YELLOW}sudo bash scripts/setup_mosquitto.sh${NC}"
    echo "2. Flash the strip with WLED (install.wled.me) and point its MQTT at the"
    echo "   broker; register your devices in ${YELLOW}config/devices.yaml${NC}."
    echo "3. Start Ollama: ${YELLOW}ollama serve${NC}  (model: $OLLAMA_MODEL)"
    echo "4. Start Clank: ${YELLOW}./start_clank.sh${NC}  (say \"hey clank ...\")"
    echo
